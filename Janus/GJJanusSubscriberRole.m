...
@interface GJJanusSubscriberRole()
{
    RTCVideoTrack* _videoTrack;
    CGSize  _renderSize;
}
@end
@implementation GJJanusSubscriberRole
@dynamic delegate;
+(instancetype)roleWithDic:(NSDictionary*)dic janus:(GJJanus*)janus delegate:(id<GJJanusRoleDelegate>)delegate{
    GJJanusSubscriberRole* publish = [[GJJanusSubscriberRole alloc]initWithJanus:janus delegate:delegate];
    publish.ID = dic[@"id"];
...
-(instancetype)initWithJanus:(GJJanus *)janus delegate:(id<GJJanusPluginDelegate>)delegate{
    ...
    if (self) {
        self.pType = kPublishTypeSubscriber;
...
-(void)joinRoomWithRoomID:(NSString *)roomID
    ...
    if (self.attached == NO && self.status == kJanusRoleStatusDetached) {
        ...
        [self attachWithCallback:^(NSError *error) {
            if (error == nil) {
                if (wkSelf) {
                    [wkSelf joinRoomWithRoomID:roomID
                                       display:display
                                         appId:appId
                                         token:token
                                         block:block];
    ...
    [super joinRoomWithRoomID:roomID display:display appId:appId token:token block:block];
}
...
-(void)videoView:(id<RTCVideoRenderer>)videoView didChangeVideoSize:(CGSize)size{
    if (CGSizeEqualToSize(_renderSize, CGSizeZero)) {
        _renderSize = size;
        [self.delegate janusSubscriberRole:self firstRenderWithSize:size];
    }else{
        [self.delegate janusSubscriberRole:self renderSizeChangeWithSize:size];
    }
}
